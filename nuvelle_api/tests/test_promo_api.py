...
def test_promo_job_file_download_redirects_to_signed_url(
    ...
    response = client.get(
        "/api/v1/promo/jobs/signed-job/files/teaser.mp4/download",
        follow_redirects=False,
    )

    assert response.status_code == 302
    assert response.headers["location"] == "https://storage.example/signed-teaser"
    assert seen == {
