#!/usr/bin/env python3
"""
Nuvelle promo-shorts service — local HTTP wrapper around the nuvelle_kit package.
...
Run:  python3 -m nuvelle_kit.promo_server  # listens on 127.0.0.1:8799
...
def run_job(job_id, src_path, title, ep, dur, beats=None, prompt="", cover_image=""):
    ...
    try:
        ...
        j["log"] = "generated by nuvelle_kit package"
        slug = result.output_dir.name or (slugify(title) + f"_e{ep}")
        tt_safe, tt_notes, cover_warn = True, "", ""
        if result.plan_path.exists():
            try:
                pl = json.load(open(result.plan_path))
                tt_safe = pl.get("tt_safe", True)
                tt_notes = pl.get("tt_notes", "")
                cover_warn = pl.get("cover_warn", "")
            except Exception:
                pass
        j.update(status="done", slug=slug,
                 files={"teaser": f"/file?slug={slug}&n=teaser.mp4",
                        "cover": f"/file?slug={slug}&n=cover.jpg"},
                 caption=result.caption_text, title=title, tt_safe=tt_safe, tt_notes=tt_notes, cover_warn=cover_warn)
